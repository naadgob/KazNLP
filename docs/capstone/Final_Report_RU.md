...
**Методология:** CRISP-DM. **Основной артефакт:** `main.ipynb` (274 cells, 7 глав + §10 сравнение моделей, вкл. HeLI/heliport + windows grid).
...
*Единый hold-out:* `data/training/filter/v1/test.csv`. Источник: `main.ipynb` §10 (cells 265–273). HeLI/heliport добавлен после рекомендации Tommi Jauhiainen: нейтрализовать заимствования, затем overlapping windows с коротким grid (`scripts/heli_lid.py`, список `data/processed/heli_loanwords_v1.txt`; best окна 2+3, min_count=1).
